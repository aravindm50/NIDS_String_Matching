""" @Author: Aravind Madan Mohan Kannan Abinaya
 Student ID: A0164966M   Student ID: A0186641Y"""
...
class ACAuto(Trie):
	...
	def report_all_matches(self, text):
		...
		for character in text:
			# If current node has character as a child, go to the next node
			...
			if (current_node.word is not None):
				if str(current_node.word) not in matches.keys():
					matches[str(current_node.word)] = [pos - len(current_node.word) + 1]
				else:
					matches[str(current_node.word)].append(pos - len(current_node.word) + 1)
			output_searcher = current_node.ACoutput_link
			while (output_searcher is not None):
				if str(output_searcher.word) not in matches.keys():
					matches[str(output_searcher.word)] = [pos - len(output_searcher.word) + 1]
				else:
					matches[str(output_searcher.word)].append(pos - len(output_searcher.word) + 1)
		...
		while (i < len(text)):
			...
			while self.node_has_child(v, char_to_find) and (i - j >= 0):
				...
				if (v.word is not None):
					if str(v.word[::-1]) not in matches.keys():
						matches[str(v.word[::-1])] = [i - j + 1]
					else:
						matches[str(v.word[::-1])].append(i - j + 1)
...
class PFACAuto(PFACTrie):
	def create_node(self, character, depth, parent):
		return PFACNode(character, depth, parent)
