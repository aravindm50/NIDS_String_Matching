...
num_pat = 50
while (firstloop != 1):
    rs = [rstr.rstr(string.ascii_uppercase, 5,10) for j in range(num_pat)]
    with open('Varying_random_file.csv', mode='w') as random1:
...
a =[]
with open('Varying_random_file.csv', newline='\n') as csvfile:
    data = list(csv.reader(csvfile))
n = 50000
with open ('dataset2.csv', mode = 'w') as dataset:
    for z in range(100):
